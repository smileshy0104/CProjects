...
class GoodGay {
 public:
  ...
  // visit 方法，作为好友函数可以访问 Building 类的私有成员
  void visit();

  // visit1 方法，不是好友函数，不能访问 Building 类的私有成员
  void visit1();

  // 指向 Building 对象的指针，GoodGay 可以通过这个指针访问 Building 的成员
  Building* builidng;  // 注意：这里拼写错误，应该是 building
};

// 定义 Building 类，表示一栋房子
class Building {
  // 声明 GoodGay 类中的 visit 成员函数是 Building 类的好友（friend）
  // 这意味着 visit 函数可以访问 Building 类的私有成员
...
// 在类外定义 GoodGay 的构造函数
GoodGay::GoodGay() {
  builidng = new Building;  // 注意：这里拼写错误，应该是 building
...
void GoodGay::visit() {
  ...
  // 访问并输出房子的私密区域（卧室）
  // 由于 visit 是 Building 的好友函数，因此可以访问私有成员 m_BedRoom
  cout << "visit正在访问" << builidng->m_BedRoom << endl;
}

// 在类外定义 GoodGay 的 visit1 方法
void GoodGay::visit1() {
  // 访问并输出房子的公共区域（客厅）
  // visit1 不是好友函数，只能访问公共成员
  cout << "visit1正在访问" << builidng->m_SittingRoom << endl;
}

// 测试函数，用于创建 GoodGay 对象并调用其方法
void test() {
  GoodGay gg;   // 创建一个 GoodGay 对象
  gg.visit();   // 调用 visit 方法访问 Building 对象的属性
  gg.visit1();  // 调用 visit1 方法访问 Building 对象的公共属性
}
