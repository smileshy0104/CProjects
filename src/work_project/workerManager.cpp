...
WorkerManager::WorkerManager() {
  ...
  // 3.文件存在不为空
  int num = this->Get_EmpNum();  // 获取文件中的人数
  this->m_EmpNum = num;          // 记录人数
  // 开辟空间，当文件中的数据存到数组中
...
int WorkerManager::Get_EmpNum() {
  ...
  // 用于存储文件中员工的ID、姓名和部门ID
  int id;
  string name;
  int did;
  // 初始化计数器
  int num = 0;
  // 循环读取文件中的员工信息，每成功读取一条信息，计数器增加
...
void WorkerManager::Mod_Emp() {
  if (this->m_FileIsEmpty) {
    ...
    if (ret != -1) {
      ...
      string newname = " ";
      int newselect = 0;
      cout << "查找到了编号为" << id << "的这个职工," << "请输入新的职工号"
           << endl;
      cin >> newid;
      cout << "请输入新的姓名" << endl;
      cin >> newname;
...
void WorkerManager::Clean_File() {
  ...
  if (select == 1) {
    ...
    if (this->m_EmpArray != NULL) {
      // 删除堆区的每个对象
      for (int i = 0; i < this->m_EmpNum; i++) {
        delete this->m_EmpArray[i];  // 释放堆区空间
      }
      // 删除堆区数组指针
      delete this->m_EmpArray;
      // 重置成员变量
